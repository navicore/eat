...
static __always_inline int extract_interval_id(struct __sk_buff *skb, 
                                               ...
                                               int payload_offset) {
    char buf[512];  // Increased to handle HTTP headers + SSE data
    int ret;
    
    // Read a larger chunk of the payload to account for HTTP headers
    ret = bpf_skb_load_bytes(skb, payload_offset, buf, sizeof(buf));
    if (ret < 0)
        return -1;
    
    // First try to find SSE data prefix
    const char sse_pattern[] = "data: ";
    int sse_pos = find_pattern(buf, sizeof(buf), sse_pattern, sizeof(sse_pattern) - 1);
    int search_start = 0;
    if (sse_pos >= 0) {
        // If we found SSE prefix, start searching after it
        search_start = sse_pos + sizeof(sse_pattern) - 1;
    }
    ...
    // Search from the appropriate starting position
    int pos = -1;
    for (int i = search_start; i <= sizeof(buf) - pattern_len && i < 480; i++) {
        int match = 1;
        for (int j = 0; j < pattern_len && j < 20; j++) {
...
int tc_audio_trace(struct __sk_buff *skb) {
    ...
    if (payload_len < 50)  // Lower threshold, headers might be in separate packet
        return TC_ACT_OK;
